...
const User = ({ id, onCancel, reload }: UserProps) => {
  ...
  const onFinish = async (values: Record<string, any>) => {
    ...
    Object.keys(time).map((item) => {
      // 对时间变量需要格式化
      params[item] = (time as Record<string, moment.Moment>)[item].format();
    });
    try {
  ...
  return (
    <Form {...layout} form={form} onFinish={onFinish} onFinishFailed={onFinishFailed} initialValues={initialValues}>
      <Form.Item label="Username" name="username" rules={[{ required: true, message: 'Please input your username!' }]}>
        <Input allowClear />
      </Form.Item>
      <Form.Item name="gender" label="Gender" rules={[{ required: true }]}>
        <Select onChange={onGenderChange} allowClear options={SelectOptions} />
      </Form.Item>
      <Form.Item label="BirthDate" style={{ marginBottom: 0 }}>
        <Form.Item name={['time', 'year']} rules={[{ required: true }]} style={{ display: 'inline-block', width: 'calc(50% - 4px)', marginRight: 8 }}>
          <DatePicker style={{ width: '100%' }} />
        </Form.Item>
        <Form.Item name={['time', 'month']} rules={[{ required: true }]} style={{ display: 'inline-block', width: 'calc(50% - 4px)' }}>
          <DatePicker style={{ width: '100%' }} />
        </Form.Item>
      </Form.Item>
